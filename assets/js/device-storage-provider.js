(function installWebWindowsStorageProvider(global) {
  ...
  function createBrowserAdapter(emit) {
    ...
    return {
      ...
      async pickDirectory(options) {
        ...
        const replacementId = String(options?.replaceVolumeId || "");
        const id = replacementId && memoryHandles.has(replacementId) ? replacementId : randomId("browser-dir");
      ...
      async listDirectory(volumeId, path) {
        ...
        for await (const [name, handle] of resolved.handle.entries()) {
          ...
          entries.push(Object.assign(browserMetadata(handle, file), {
            path: [...resolved.parts, name],
            readable: resolved.permission.readable,
            writable: resolved.permission.writable
          }));
        }
  ...
  function createAndroidAdapter(bridge, emit) {
    ...
    return {
      ...
      async pickDirectory(options) {
        const result = await call("storagePickDirectory", {
          writable: options?.writable !== false,
          replaceVolumeId: options?.replaceVolumeId || null
        });
        emit?.("webwindows:storage-change", { reason: "directory-picked", volume: result });
        return result;
      },
