...
var aiBody=$('#ai-body'), aiInput=$('#ai-input'), aiSend=$('#ai-send');
  ...
  // 文件检索工具（ai-file-tools.js 注册到 WebWindows.aiTools）。
  // 这些函数原本在 529385c「feat: connect DeskTalk to unified file search」里，
  // 但 T-006 的日期权威合并只取了 file-query-parser.js / ai-file-tools.js 的 blob，
  // 没有带上这一侧，桌讯因此从未向模型发送过 tools，工具注册了却无人调用。
  ...
  // 只在模型真的请求检索、且用户这句话确实像找文件时才执行工具。
  // 检索结果只渲染进界面，不回传模型（与 API 文档「不把结果混入确定性回答」一致）。
  function handleAIToolCalls(data, userText){
    ...
    if(!window.WebWindows || !window.WebWindows.aiTools) return Promise.resolve(false);
...
function sendAI(){
  ...
  requestAI(payload, 0).then(function(data){
    return handleAIToolCalls(data,v).then(function(handled){
      if(handled) return;
      // 模型仍然要了工具但我们没有执行：去掉 tools 重试一次，避免把工具调用当答案显示。
      var calls=extractAIToolCalls(data);
      if(calls.length){
