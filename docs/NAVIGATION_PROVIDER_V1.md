...
自动模式通过同源代理按地区选择供应商：中国大陆优先高德、百度备用，其他地区使用 Google Routes。驾车、公交、步行和骑行结果均在问道内绘制，不再打开外部地图。无密钥社区模式仍使用 OpenFreeMap Liberty 矢量底图、Photon 地理编码/附近地点和 OSRM 驾车路线；社区服务不支持的方式会明确显示不可用，不绘制虚假的直线路线。费用卡只显示供应商返回的金额，不会臆测高速费、IC 卡或现金票价。
