...
// 生产版问道已移除“发送到外部地图”的出口：不再拼 Google Maps 链接，
// 也不再 window.open 起终点。这里反向断言该出口不会回归。
assert.doesNotMatch(appSource,/google\.com\/maps\/dir/);
assert.match(appSource,/function handleRouteSearch/);
assert.doesNotMatch(appSource,/window\.open\(external\.href/);
