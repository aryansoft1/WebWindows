...
function bindWindowBehavior(winEl){
  ...
  const availableBounds = () => {
    ...
    return {
      width: Math.max(0, viewport?.width || window.innerWidth),
      height: Math.max(0, (viewport?.height || window.innerHeight) - taskbarHeight),
    };
  };

  // ------- 拖拽：Pointer Events 同时覆盖鼠标、触控笔与触摸设备 -------
  // 手机/窄屏由 CSS 固定为工作区全屏，避免手指拖动把窗口甩出可视区域。
  let moving = false, dx = 0, dy = 0, movePointerId = null;
  ...
  header?.addEventListener('pointerdown', (e) => {
    ...
    moveHandler = (ev) => {
      ...
      winEl.style.left = Math.max(0, Math.min(maxLeft, ev.clientX - dx)) + 'px';
      winEl.style.top = Math.max(0, Math.min(maxTop, ev.clientY - dy)) + 'px';
    };
    stopMove = (ev) => {
