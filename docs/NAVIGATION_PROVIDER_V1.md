...
无密钥社区模式使用 OpenFreeMap Liberty 矢量底图、Photon 地理编码/附近地点和 OSRM 驾车路线。公交、步行和骑行暂时是距离估算；社区接口不返回可靠的高速通行费、IC 卡或现金票价。费用卡会明确显示“未提供”，不会臆测金额。正式公交路线和费用需要由同源代理接入已签约、支持当地票价数据的供应商。外部演示服务没有 SLA，正式商业部署应改用签约或自托管服务。断网或服务限流时会回退到随应用发布的小型地点库和直线距离估算。
