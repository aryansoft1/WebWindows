...
Dim resultLimit, sortField, sortOrder, scannedCount, resultCount
...
Sub ConsiderFile(ByVal file, ByVal relativeFolder, ByVal sourceName, ByVal nodeId)
  Dim fileName, extension, mimeType, createdEpoch, modifiedEpoch, uploadedEpoch, score, reason, pathValue
  ...
  If folderFilter <> "" And InStr(1, LCase(relativeFolder), folderFilter, vbTextCompare) = 0 Then Exit Sub
  ...
  score = FileNameScore(fileName, queryText, reason)
  ...
  AddResult sourceName, nodeId, fileName, pathValue, relativeFolder, extension, mimeType, CLng(file.Size), _
    createdEpoch, modifiedEpoch, uploadedEpoch, score, reasons
End Sub

Sub AddResult(ByVal sourceName, ByVal nodeId, ByVal fileName, ByVal pathValue, ByVal folderPath, ByVal extension, _
  ...
  resultJson(resultCount) = "{""id"":""" & CloudJson(sourceName & ":" & nodeId & ":" & pathValue) & _
    ...
    """,""name"":""" & CloudJson(fileName) & """,""path"":""" & CloudJson(pathValue) & _
...
Function FileNameScore(ByVal fileName, ByVal wanted, ByRef reason)
  ...
  If InStr(1, value, wanted, vbTextCompare) > 0 Then reason = "fileNameContains": FileNameScore = 78: Exit Function
  tokens = Split(wanted, " "): allTokens = True
  For Each token In tokens
    If token <> "" And InStr(1, value, token, vbTextCompare) = 0 Then allTokens = False
  Next
