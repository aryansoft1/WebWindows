# Storage Capability v1 — Android Real-device Validation Record

Status: **Storage Capability v1 Frozen**. Final adjudication accepted the
validation matrix with four documented validation-environment limitations.
This document does not change the public API, Native ABI, Native Bridge v1,
RuntimeInfo, or production behavior.

## Current device baseline

- Android 15 tablet; Android System WebView `151.0.7922.170`.
- Launcher commit `746ff3369d35fbe755900adef558261dc28ba5c7`.
- Debug APK SHA-256
  `749A4B349A9C04A312E1120D48605721CC8449E3D5DEDB9111471D588A16C247`.
- Production Storage provider SHA-256
  `5318CB616D738DB2BF207338EB488601BAB6C4C7383A91BA8D604477DF5263E4`;
  asset token `20260825-storage-2`.
- SAF provider label in this record: Android system local-storage picker. No
  provider authority or authorization identifier is recorded.

## Contract lock

- Native Bridge v1 is **Frozen**.
- Battery, Network, Display, and Audio contracts are unchanged.
- Storage v1 is **Frozen**.
- Storage write, streaming, and forget/release are unsupported.
- The maximum read is exactly **8,388,608 raw bytes**.
- Volumes use opaque IDs and exact-origin isolation.
- Public results must never expose URI, filesystem path, document ID, or
  provider authority.

Use `https://www.y0.hk` as origin A and `https://y0.hk` as origin B. For every
case, record Android version, WebView version, launcher build, provider name,
origin, test-file hash/size, result time, and evidence path. Mark **Actual**
and **Status** only during the run: `Pass`, `Fail`, `Blocked`, or `N/A`.

## Evidence legend

- **Capture**: screenshot or screen recording.
- **API record**: redacted console/inspection record containing public result or
  error code only; never record a `content://` URI.
- **System record**: Android settings/provider view used to establish grant or
  revocation state.
- **Log observation**: optional filtered launcher log; redact URI/path/provider
  identifiers before attaching it.

## Matrix

| ID | Case and precondition | Steps | Expected / failure code | Automated evidence | Required device evidence | Actual | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| RD-01 | SAF picker success; trusted A, no active picker | Call `pickDirectory`, select readable directory | One A-scoped opaque volume; no URI leak | Contract/lifecycle smoke | Capture + API record | Android 15; WebView 151.0.7922.85; Launcher `6c6480a`; Android SAF local-storage UI; origin A; fixture N/A. At 2026-08-25 17:41:30 +09:00, picker resolved to one redacted opaque `directory` volume (`source:android-saf`); grant was readable/persisted, and `listVolumes()` changed 0→1. Public records contained no URI/path/provider identifier. Evidence: operator-local picker capture + redacted CDP API record in this validation task. | Pass |
| RD-02 | Picker cancel | Open picker, press Back/Cancel | `user-cancelled`; no record changes | Cancellation path smoke | Capture + before/after list | Same environment/provider/origin as RD-01; fixture N/A. At 2026-08-25 17:43:38 +09:00, Back/Cancel rejected with public code `user-cancelled`; `listVolumes()` remained 1→1. Evidence: operator-local picker observation + redacted CDP before/after API record in this validation task. | Pass |
| RD-03 | Picker busy; keep first picker open | Issue second picker request | Second rejects `picker-busy`; first remains sole operation | `StoragePickerStateTest`/lifecycle smoke | API record + capture | Same environment/provider/origin as RD-01; fixture N/A. Second concurrent request rejected in 92 ms with public code `picker-busy`; first picker remained active, then user cancellation returned `user-cancelled`; `listVolumes()` remained 1→1 (completed 2026-08-25 17:44:32 +09:00). Evidence: operator-local picker observation + redacted CDP API record in this validation task. | Pass |
| RD-04 | Picker then navigation; A and B pages available | Open on A, navigate to B, finish old picker | B receives no result; no grant/mapping persisted; later picker works after callback | Tombstone lifecycle smoke | Screen recording + before/after lists | Re-tested on Android 15 / WebView 151.0.7922.170 with Launcher `746ff33` and production Android SAF provider. With A=4 and B=1 at the controlled baseline, an A picker selected a previously ungranted directory only after navigation invalidated its generation: A remained 4→4, B remained 1→1, and neither origin listed the stale target. Reauthorization stale-result testing preserved the existing `WebWindowsValidation` mapping and created no target mapping. No old public result/event reached B; a subsequent picker opened normally and cancellation returned `user-cancelled` with A unchanged. No URI/path/provider identifier or opaque ID was recorded. Evidence: operator-local picker captures + redacted CDP/API records in this validation task. | Pass |
| RD-05 | Timeout/late callback | Leave picker open past host deadline, then select | Old result discarded; no grant/reply; note whether tombstone blocks until callback | Timeout state and bootstrap smoke | Timed recording + API record | Current device baseline; origin `https://y0.hk`; 2026-08-25. Valid run started at epoch ms `1787656289864`; native deadline was `1787656404864`; public JS rejection arrived after 120,172 ms with `timeout`. A late successful picker result returned after 129,487 ms but emitted no Storage event and created no mapping: origin counts stayed 4→4 and the comparison origin stayed 1→1. A subsequent picker opened normally and cancellation returned `user-cancelled`, again with no count change. The earlier attempt invalidated by the device's screen timeout is not evidence. Fixture N/A. Evidence: operator-local timed picker observation + redacted CDP/API record in this validation task. | Pass |
| RD-06 | Exact-origin isolation | A authorizes dir A; B lists/reads/reauthorizes; B authorizes dir B; return A | Each origin sees and operates only own volume; foreign ID is not found | Origin-key/lifecycle smoke | API records for list/metadata/open/reauthorize | Current device baseline; Android system local-storage picker; origins `https://y0.hk` and `https://www.y0.hk`; 2026-08-25. Each origin authorized and read its own 17-byte `probe.bin`: SHA-256 `3A353FBF3ACDDCAD2D8C5C8BBD9571769373523FE9AE19A345FF3F3089ADA4F9` and `933351E871C67DF7C6905EEF4E5864411E03694EA1907746A0FD4043E2A3292A`. In both directions the foreign opaque ID was absent from `listVolumes()` and metadata/open/replace each rejected `storage-volume-not-found`; list counts were unchanged. One origin already had an independently authorized same-name directory, but strict ID comparison confirmed it was not the foreign mapping. Evidence: operator-local picker observations + redacted CDP/API records in this validation task. | Pass |
| RD-07 | Persisted grant restore | Authorize, force-stop/restart launcher | Same origin restores v2 volume and remains readable | Unit policy coverage | Capture + API records before/after restart | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-25. Before force-stop the origin listed 2 volumes. After a real Launcher force-stop and restart it again listed the same 2 volumes; an in-memory comparison (no ID logged or persisted) confirmed the complete opaque-ID set was unchanged. The restored 17-byte fixture remained readable with SHA-256 `3A353FBF3ACDDCAD2D8C5C8BBD9571769373523FE9AE19A345FF3F3089ADA4F9`. Evidence: operator-local restart observation + redacted CDP/API before/after record in this validation task. | Pass |
| RD-08 | Revoked grant | Authorize then revoke through available system/provider control | Volume stays listable; list/read/metadata reject permission-revoked; reauth is possible | Error-path/provider smoke | System record + API records | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-25. Android App Info exposed ordinary permissions, storage/cache, and force-stop controls but no per-app persisted SAF-grant manager; the available Android activity command interface likewise exposed no URI-grant revoke operation. Clearing app data would also delete mappings, and editing internal state or deleting a directory would not be a genuine persisted-grant revoke, so none was performed. No public error code was fabricated. Evidence: operator-local Android settings inspection + redacted command-capability check in this validation task. | Blocked |
| RD-09 | Same-origin reauthorize | Existing A v2 volume | `pickDirectory({replaceVolumeId:id})`, select a directory | Same opaque ID; name/handle/grant reflect selection; cancel preserves old mapping | Browser unit parity; Android state tests | Before/after API record + capture | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-25. Cancel path returned `user-cancelled`, kept count 2→2, preserved the original opaque ID/name, and retained the original 17-byte fixture hash `3A353FBF3ACDDCAD2D8C5C8BBD9571769373523FE9AE19A345FF3F3089ADA4F9`. Success path selected `WebWindowsReauthA`, kept count 2→2, returned the exact same opaque ID, updated the public name/handle, and read the new 17-byte fixture with SHA-256 `9CE744CDA9753638D6515372E2494BEBBF3F7D806689A9E4B083A3C469428200`. Evidence: operator-local picker observations + redacted CDP/API records in this validation task. | Pass |
| RD-10 | Multiple SAF providers | DocumentsUI/Downloads, local shared storage, Google Drive if installed, other available provider | Same public schema; unavailable fields become `null`, not invented | Schema/validator smoke | Provider-labelled records | Current device baseline; origin `https://y0.hk`; 2026-08-26. Tested (1) Android system local shared-storage picker with `WebWindowsReauthA`, (2) the Downloads location through the same local DocumentsProvider with `WebWindowsDownloadsRD10`, and (3) the already configured Google Drive provider with a dedicated empty `WebWindowsValidationRD10` directory created through the standard picker UI. All returned an opaque non-platform ID, `kind:directory`, granted readable/writable persisted permission, and `source:android-saf`; root metadata and `listDirectory()` conformed to the public schema. Downloads returned one 27-byte `text/plain` file and exact read SHA-256 `2555411BF3479D951C9B0CB2A638FEBF9EF57903101F95A5B2FA59FA6A19AD28`; Drive returned an empty list. Drive/provider roots that Android marks unselectable were not treated as failures; the dedicated subdirectory was selectable. No other natural provider was shown, and none was installed for the test. No private provider/account/file name or authorization identifier is retained. Evidence: operator-local picker observations + redacted CDP/API records in this validation task. | Pass |
| RD-11 | Root directory metadata | Authorized volume | `getMetadata(id, [])` | Directory metadata, `path: []`, size/type `null` | Provider schema smoke | API record | Current device baseline; Android system local-storage picker and Google Drive; origin `https://y0.hk`; 2026-08-26. Local public object: `{supported:true,name:"WebWindowsReauthA",kind:"directory",size:null,type:null,lastModified:<non-negative integer>,readable:true,writable:true,path:[],source:"android-saf"}`. Drive returned the same schema and null directory size/type for the dedicated validation directory. Display names were safe; no native/provider-private identifier appeared. Evidence: redacted CDP/API records in this validation task. | Pass |
| RD-12 | Nested directory | Directory with nested child | List and metadata at nested relative path | Safe relative `string[]`; correct child path | Browser/provider smoke | API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. `listDirectory(volumeId,["nested"])` returned normal `child.txt` and safe Unicode `雪.txt`, each with the exact two-segment public path `['nested',name]`, `kind:file`, size 21, `type:text/plain`, non-negative `lastModified`, boolean readable/writable, and `source:android-saf`. Passing each returned path back to `getMetadata()` reproduced the same child. No absolute/slash-joined path was exposed. Fixture SHA-256 values: `A3039E1845782E687508166638BC72215AE33884897C6C3D85543B9B790357C4` and `156879F07EDCAFC0F9AA91742B7BC4249B0F8BFE31C9C30A08E8966BC684E9B6`. Evidence: redacted CDP/API records in this validation task. | Pass |
| RD-13 | File metadata | Known file | `getMetadata(id,[name])` | `kind:file`; non-negative known values or `null` | Metadata contract smoke | API record | Current device baseline; Android system local-storage and Downloads locations; origin `https://y0.hk`; 2026-08-26. Normal file metadata reported `supported:true`, the correct safe name and segment path, `kind:file`, exact fixture sizes (21 and 27 bytes), `type:text/plain`, non-negative integer `lastModified`, boolean readable/writable, and `source:android-saf`. Known sizes matched the raw bytes and the Downloads read hash. No tested file provider returned unknown nullable metadata in this block. Evidence: redacted CDP/API records in this validation task. | Pass |
| RD-14 | Directory metadata | Known directory | `getMetadata(id,[name])` | `kind:directory`, size/type `null` | Metadata contract smoke | API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. `getMetadata(volumeId,["nested"])` returned `{supported:true,name:"nested",kind:"directory",size:null,type:null,lastModified:<non-negative integer>,readable:true,writable:true,path:["nested"],source:"android-saf"}`. Root directories from local, Downloads, and Drive likewise kept size/type null; no directory was assigned a file byte size. No naturally exposed unsafe child name was observed during RD-10～RD-14. Evidence: redacted CDP/API records in this validation task. | Pass |
| RD-15 | Zero-byte file | Empty file | Open file | Empty `ArrayBuffer`; size `0`, not `null` | Data-contract smoke | API record + file fixture hash | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Genuine 0-byte fixture SHA-256 `E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855`. `openFile()` succeeded in approximately 230 ms with `kind:file`, metadata size `0`, public `data` as `ArrayBuffer`, and `data.byteLength:0`. Public result keys were only `data` and `metadata`. Evidence: redacted CDP/API record in this validation task. | Pass |
| RD-16 | UTF-8 file | UTF-8 fixture | Open and decode at page | Exact bytes/text are returned | Browser/read smoke | Fixture hash + API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Deterministic UTF-8 fixture contained ASCII, Chinese, Japanese, emoji, and newlines; exact size 33 bytes and SHA-256 `AC7C9BA03D335F4FC70DE3136858441C7760DEF1DDC0D46568B47F7BE25C8B84`. Public read completed in approximately 219 ms; metadata size and `ArrayBuffer.byteLength` were both 33 and the WebView-computed hash exactly matched the source fixture. Evidence: redacted CDP/API record in this validation task. | Pass |
| RD-17 | Binary/NUL file | Binary fixture containing NUL | Open and compare bytes/hash | Exact bytes; no text coercion | Base64/read contract smoke | Fixture hash + API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Deterministic 13-byte binary fixture included two NUL bytes, bytes at/above `0x80`, and a non-text sequence; SHA-256 `0586E7C692BBFD887D3920F0F527198D0834E1B65C0AC4B41CF489A8AEBFDC45`. Public read completed in approximately 237 ms; all 13 byte values and the hash matched exactly, with no NUL truncation or string coercion. Evidence: redacted CDP/API record in this validation task. | Pass |
| RD-18 | Exact 8 MiB | File exactly 8,388,608 bytes | Open file | Success; byteLength exactly 8,388,608 | Native read-limit unit test | Size/hash + API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Deterministic fixture size exactly 8,388,608 raw bytes; SHA-256 `0FF4D6C068BE24637E84EA9F481C3C29F7AFCDEF1E06E1F40A68E5DE85DCBB5B`. `openFile()` succeeded in approximately 3,734 ms; metadata size and `ArrayBuffer.byteLength` both equalled 8,388,608 and the WebView-computed hash matched. Launcher/WebView remained responsive with no process death, crash, ANR, or observed OOM. Evidence: redacted timed CDP/API record + foreground/process observation in this validation task. | Pass |
| RD-19 | 8 MiB + 1 | File of 8,388,609 bytes | Open file | `storage-file-too-large`; no page oversized decode | Native stream-limit smoke | Size/hash + API record | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Deterministic fixture size 8,388,609 raw bytes; SHA-256 `37FFE721FE437045FA7862BF403AC7DBD7EEE9545A7BC1D66898E9507CF84DDF`. Provider metadata reported the exact concrete size; `openFile()` rejected in approximately 175 ms with `storage-file-too-large`. No successful data result or oversized `ArrayBuffer` reached the page; Launcher/WebView remained responsive. Evidence: redacted timed CDP/API record + foreground/process observation in this validation task. | Pass |
| RD-20 | Provider `SIZE == null` | Provider/file for which `OpenableColumns.SIZE` is absent/null | Metadata size is `null`; bounded actual stream still decides read | Nullable metadata smoke | Provider observation + API record | Current device baseline; origin `https://y0.hk`; 2026-08-26. Naturally available Android local shared storage, Downloads, and the dedicated Google Drive validation directory were checked without modifying provider internals. Local exposed 7 files and Downloads 1 file, all with concrete sizes; the Drive validation directory was empty and supplied no real file candidate. No naturally available file exposed unknown size, so the bounded null-SIZE stream case could not be executed. Evidence: redacted provider-count/nullability scan in this validation task. | Blocked |
| RD-21 | Unknown modification time | Provider reports `lastModified == 0` or no reliable time | Public `lastModified:null` | Metadata policy unit test | Provider observation + API record | Current device baseline; origin `https://y0.hk`; 2026-08-26. The same local, Downloads, and Drive validation sources were checked. Every observed root and all 8 available test files supplied a valid non-negative modification time; Drive had no file in the dedicated directory. No genuine unknown/zero timestamp condition was naturally available, and none was manufactured. Evidence: redacted provider-count/nullability scan in this validation task. | Blocked |
| RD-22 | Large ContentResolver stream | File substantially larger than 8 MiB, including provider with unknown SIZE if possible | Native stops over limit; no crash/OOM; `storage-file-too-large` | `StorageReadPolicy` test | Timed recording + optional redacted log | Current device baseline; Android system local-storage picker; origin `https://y0.hk`; 2026-08-26. Deterministic large fixture size 16,777,339 bytes; SHA-256 `E8ECA6CCAE90EF633414EDED567B34F9F6595665E257176904769703335CD8FA`. This provider supplied concrete size, and native rejected in approximately 210 ms with `storage-file-too-large` before any successful payload reached JS. A subsequent 13-byte binary read succeeded in approximately 192 ms with the exact expected hash. Across successful reads, public result keys remained only `data` and `metadata`; `data` was an `ArrayBuffer`, with no Base64 or native/provider-private identifier exposed. Launcher remained focused and its process alive; no crash, ANR, observed OOM, or restart occurred. A real unknown-SIZE stream was unavailable as documented in RD-20. Evidence: redacted timed CDP/API record + foreground/process observation in this validation task. | Pass |
| RD-23 | HTML upload regression | Page with `<input type=file>` | Upload success/cancel work before/during/after Storage picker states; no callback cross-talk | Separate request-code lifecycle smoke | Screen recording + upload result | Fixed device baseline; Android 15; WebView 151.0.7922.170; Launcher `746ff33`; production origins `https://y0.hk` and `https://www.y0.hk`; 2026-08-26. A standard visible HTML file input was exercised with real touch for normal success and cancel, after Storage success and cancel, before a subsequent Storage cancellation, and after Storage navigation/tombstone invalidation. The successful upload returned one 33-byte `text/plain` fixture with SHA-256 `AC7C9BA03D335F4FC70DE3136858441C7760DEF1DDC0D46568B47F7BE25C8B84`; cancel returned zero files. Storage volume counts remained unchanged in every HTML flow. Storage success preserved the replacement opaque identity without recording it; Storage cancellations returned `user-cancelled`; the stale callback did not reach the navigated page or create a mapping. Both systems remained usable, with no callback cross-talk or request-code collision. Initial attempts lacking a real user gesture or using an insufficient observer window were discarded and are not evidence. Evidence: operator-local picker observations + redacted CDP/API records in this validation task. | Pass |
| RD-24 | HOME Launcher regression | Normal production launch | HOME/loading sequence returns and Storage actions do not disrupt it | Device experience smoke (web) | Capture | Fixed device baseline; Android 15; WebView 151.0.7922.170; Launcher `746ff33`; production origin `https://www.y0.hk`; 2026-08-26. After a real force-stop and HOME cold start, the Launcher process restarted with a new PID and loaded the production manifest `2026.08.26.1`; by approximately T+6 s the document was complete and remained stable through T+11 s. A Storage picker opened and cancellation returned `user-cancelled`. Android Settings was then brought to the foreground; HOME returned to the Launcher with the production document complete, Runtime available, the origin's six Storage mappings intact, and write still unsupported. Storage reopened after resume and canceled normally. No blank page, launch loop, stale picker contamination, crash, or ANR was observed. Evidence: operator-local HOME/background/picker observations + redacted process/CDP/API records in this validation task. | Pass |
| RD-25 | Device capability regression | Usable battery/network/display/audio conditions | Verify battery/charging, Wi-Fi/network, brightness, volume and physical-key refresh | Dedicated capability suites | Capture + API records | Fixed real-device evidence remains as recorded on Android 15 / WebView 151.0.7922.170 / Launcher `746ff33`: after a Storage cancellation, Battery and Network refreshed plausibly; Display read/set/refresh/restore and Audio read/set/refresh/restore worked; Storage write remained unsupported. Supplementary validation on 2026-09-03 used an Android 16/API 36 emulator, WebView 133.0.6943.137, and the exact baseline APK SHA-256 `749A4B349A9C04A312E1120D48605721CC8449E3D5DEDB9111471D588A16C247`. Android-adapter Battery/Network reads succeeded; Display changed 0.4→0.6→0.4; Audio changed 5/15→9/15→5/15 through the public API and each refresh matched. The AVD did not route an injected volume-down key or `media_session` adjustment to STREAM_MUSIC even with the stream active, so no physical-key refresh result is claimed. The public capability contract is operational, but the specifically required physical-device key subcase remains unavailable. Evidence: redacted emulator system/CDP/API records in this validation task. | Blocked |
| RD-26 | Cache/update regression | Normal production build URL | Verify cache/offline and update flow | Existing web regression suites | Capture + version/result record | Production provenance was repaired on 2026-09-03 by uploading only `/assets/js/device-storage-provider.js` from the committed LF source with FTP binary/raw `curl --upload-file` and no ASCII/text conversion. Pre-deploy www/non-www were 21,567 bytes with SHA-256 `F08E943A940D40FB1070630D4457C7645F096516791E5C3D1AC05CB67562834E`; post-deploy both were byte-identical at 21,102 bytes with canonical SHA-256 `5318CB616D738DB2BF207338EB488601BAB6C4C7383A91BA8D604477DF5263E4`, token `20260825-storage-2`, ETag `"9e88879613bdd1:0"`, and Last-Modified `Thu, 03 Sep 2026 05:02:52 GMT`. Root cause was proven: the prior deployment branch lacked the later `*.js text eol=lf` attribute while system Git had `core.autocrlf=true`, so its clean working-tree file contained 465 CRLF conversions; FTP itself performed a raw upload. No index, manifest, contract, Android, or other production file was changed during restoration. The retry used an Android 16/API 36 emulator, WebView 133.0.6943.137, Launcher `746ff33`, and the exact baseline APK SHA above. Online production manifest `2026.09.01.6` loaded normally with Android adapter, canonical provider SHA, write `{supported:false,source:"unsupported"}`, and zero Service Worker registrations/Cache Storage keys; the existing manual update check returned `{started:true}`. With emulator airplane mode enabled, a cold start loaded the last successful production page from the existing HTTP cache with `navigator.onLine:false`, the same provider token, Android adapter, and write false. Reconnection automatically loaded a fresh online URL; canonical SHA, token, write capability, zero service-worker/cache state, and update check all remained correct. A later cold start selected Browser adapter once because Native was absent, but a page reload restored Native and a second independent cold start did not reproduce the condition; no crash or ANR was observed. Evidence: redacted binary fingerprints + emulator/CDP/API records in this validation task. | Pass |

## Unsafe provider-name probe

Where a provider can expose such entries, test empty name, `.`, `..`, slash,
backslash, NUL, Unicode, emoji, combining characters, and a long name. Unicode
that remains a safe single segment is valid. Any unsafe child/root name must
make the operation fail; it must never be silently sanitized into a different
addressable name. Current implementation reports an availability-style native
failure before public schema reconstruction rather than fabricating a uniform
entry. Record the exact public code observed; do not reclassify it in this
validation document.

## What automation covers

Automated tests demonstrate the fixed method set, schemas, canonical Base64,
metadata/payload consistency, 8 MiB stream guard, Browser reload before
replacement-ID lookup, origin-key construction, picker tombstone/busy/cancel
logic, timeout state, stale callback ordering, and independent HTML upload
request-code handling. Website suites additionally cover Runtime, Battery,
Network, Display, Audio, device experience, and Storage provider contracts.

Automation cannot establish real `DocumentProvider` metadata quality, Android
permission revocation UX, activity/background/rotation behavior, actual picker
callback timing, persisted grants across a real process restart, OS memory
behavior, physical controls, cache/update, or production launch behavior.
RD-01 through RD-26 therefore retain a real-device evidence requirement even
when an automated column exists.

## Readiness and diagnostic limits

The current code/docs/tests support all matrix operations through the public
Device API and existing launcher behavior. No production diagnostic hook is
required to execute them. The only environment limitation found during
preparation is that this workstation lacks `JAVA_HOME` and `java`, so Gradle
JVM tests/debug assembly cannot be rerun here until a JDK is supplied. This is
an environment/test-runner limitation, not a Storage contract failure.

For provider or lifecycle failures, preserve public error code plus redacted
capture. Do not add URI logging or a debugging API solely for validation.

## Canonical Frozen identity and contract

The sole canonical deployed Storage provider identity is:

- file: `assets/js/device-storage-provider.js`;
- size: **21,102 bytes**;
- SHA-256:
  `5318CB616D738DB2BF207338EB488601BAB6C4C7383A91BA8D604477DF5263E4`;
- release token: `20260825-storage-2`.

The Frozen public API is `storage.isSupported()`, `storage.getCapabilities()`,
`storage.getState()`, `storage.refresh()`, `storage.listVolumes()`,
`storage.pickDirectory()`, `storage.requestPermission()`,
`storage.listDirectory()`, `storage.openFile()`, and `storage.getMetadata()`.
The Frozen Native read contract is `storageListVolumes`,
`storagePickDirectory`, `storageListDirectory`, `storageGetMetadata`, and
`storageOpenFile`.

Frozen semantics are exact-origin ownership/isolation; opaque volume IDs;
strict path-segment, Volume, Metadata, and canonical Base64 validation; public
`ArrayBuffer` mapping; an exact whole-file maximum of 8,388,608 raw bytes
(8 MiB succeeds and 8 MiB + 1 is rejected); picker lifecycle/tombstone and
timeout behavior; public error normalization; and no URI, filesystem path,
document ID, provider-private value, or platform exception leakage. Write and
streaming remain unsupported, and the contract assumes no new Native method.

The historical 21,567-byte CRLF payload with SHA-256
`F08E943A940D40FB1070630D4457C7645F096516791E5C3D1AC05CB67562834E`
is provenance-drift evidence only and is not a second Frozen identity. The
drift arose because a clean deployment worktree used `core.autocrlf=true`
without that branch's required line-ending protection. FTP
`curl --upload-file` transferred raw bytes and did not cause the conversion.

## Accepted validation-environment limitations

The four Blocked rows remain Blocked, not Pass, Fail, waived defects, or known
product bugs:

- RD-08: the tested environment exposed no safe, natural mechanism to revoke
  one persisted SAF grant without manipulating app/provider internal state.
- RD-20: no tested real provider naturally returned an unknown/null file size.
- RD-21: no tested real provider naturally returned an unknown/zero
  modification time.
- RD-25: Battery, Network, Display, and Audio read/set/refresh passed, but the
  emulator could not substitute for a physical STREAM_MUSIC volume-key event.

A future provider exposing unknown size/time, a device exposing a safe
persisted-grant revoke path, or a physical Android volume key may close these
evidence gaps opportunistically without reopening Frozen Storage v1. Any
future behavior that violates the Frozen contract is triaged separately as a
defect.

One emulator cold start temporarily selected the Browser adapter and recovered
after reload; a subsequent independent cold start did not reproduce it. This
is retained only as a non-reproducible Native Runtime bootstrap/lifecycle
observation, not as a Storage defect, Pass condition, or resolved defect.

## Freeze gate

Final Freeze Gate: **PASS — accepted with documented validation limitations**.
The preserved matrix is **22 Pass, 0 Fail, 4 Blocked, 0 N/A**. Final GPT
adjudication promotes Storage v1 from Frozen Candidate to **Frozen** without
rewriting any Blocked row as Pass.
