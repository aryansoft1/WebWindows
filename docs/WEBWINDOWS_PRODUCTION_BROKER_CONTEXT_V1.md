...
Phase 2D.5 constructs a Host-owned, immutable, session-only authorization context after exact production package verification. Phase 2D.6 may consume it for the Battery-only Broker when the Host deployment feature gate is enabled; persistent grants, consent UI, and all other capabilities remain disabled.
