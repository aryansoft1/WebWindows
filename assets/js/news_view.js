      ...
      fetch("getPrevNextNews.asp?dir=prev&id=" + encodeURIComponent(id), { credentials: "same-origin" })
        ...
        .then(d => {
          if (!d.error) {
            ...
            const link = document.createElement("a"); link.href = `news_view.html?id=${encodeURIComponent(d.id)}&v=20260826-news-nav-1`; link.textContent = d.title; host.appendChild(link)
          }
        })
