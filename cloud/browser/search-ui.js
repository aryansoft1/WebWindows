(function installCloudSearchUI(global) {
  ...
  function initialize(){
    ...
    async function searchNow(){const query=input.value.trim();if(!query){restore();return}const request=++serial;controller?.abort();controller=new AbortController();clear.hidden=false;directory.hidden=true;view.hidden=false;view.innerHTML=`<div class="file-search-loading">${t("loading")}</div>`;try{const payload=await global.WebWindows.files.search(query,{signal:controller.signal});if(request===serial)render(view,payload,query)}catch(error){if(error?.name!=="AbortError"&&request===serial)view.textContent=error.message}}
    function schedule(){clearTimeout(timer);if(!input.value.trim()){restore();return}timer=global.setTimeout(searchNow,DEBOUNCE_MS)}
    form.addEventListener("submit",event=>{event.preventDefault();clearTimeout(timer);searchNow()});input.addEventListener("input",schedule);clear.addEventListener("click",()=>{restore();input.focus()});
  }
