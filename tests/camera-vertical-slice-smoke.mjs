...
assert.match(html, /assets\/js\/webwindows-message\.js/);
...
assert.match(appSource, /api\.saveBlob/);
assert.match(appSource, /systemDialog/);
assert.match(appSource, /api\?\.confirm/);
assert.match(appSource, /await confirmWithSystemDialog/);
assert.match(appSource, /title:\s*t\("从云资料选择图片"\)/);
assert.doesNotMatch(appSource, /(?:^|[^.\w])confirm\s*\(/m);
assert.doesNotMatch(appSource, /(?:^|[^.\w])alert\s*\(/m);
assert.match(appSource, /TranslatorApi\.availability/);
assert.match(appSource, /downloadprogress/);
assert.match(appSource, /180000/);
assert.match(appSource, /requestFullscreen/);
assert.match(appSource, /exitFullscreen/);
assert.match(appSource, /camera-capture-mode/);
assert.match(appSource, /state\.captureMode/);
assert.match(appSource, /fullscreenchange", renderCaptureMode/);
assert.doesNotMatch(appSource, /showOpenFilePicker|showDirectoryPicker/);
assert.doesNotMatch(appSource, /createObjectURL|\.download\s*=/);
assert.match(cameraCss, /:fullscreen/);
assert.match(cameraCss, /object-fit:\s*cover/);
assert.match(cameraCss, /safe-area-inset-bottom/);
assert.match(windowItem, /camera; fullscreen; translator/);
assert.match(legacyWindow, /camera; fullscreen; translator/);
