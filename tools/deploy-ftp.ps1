...
try {
  ...
  $productionManifestHash = (Get-FileHash -LiteralPath $productionManifestProbe -Algorithm SHA256).Hash.ToLowerInvariant()
...
foreach ($relative in $uploadFiles) {
  ...
  if ($relative -eq "deploy/ftp-manifest.json" -and $downloaded) {
    $actualManifestHash = (Get-FileHash -LiteralPath $backup -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actualManifestHash -ne $productionManifestHash) { throw "Production deployment manifest changed during backup." }
  }
  if ($productionEntry) {
    ...
    if ($actual -ne [string]$productionEntry.Value.sha256) {
      ...
      } elseif ($adoptChanged.Contains($relative)) {
...
$late = @("data/apps/system-apps.json", "api/function-catalog.asp", "SystemManager/index.html", "deploy/ftp-manifest.json", "index.html")
